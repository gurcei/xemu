...
void m65mon_do_next ( void )
{
	if (paused) {
		...
		trace_next_trigger = 3; // if JSR, then trigger until RTS to next_addr
...
int main ( int argc, char **argv )
{
	...
	for (;;) {
			...
			if (trace_next_trigger == 3)
			{
				trace_next_trigger--;
			}
			else if (trace_next_trigger == 2)
			{
				if (cpu65.op == 0x20)
					trace_next_trigger = 1;
				else
				{
					trace_next_trigger = 0;
					paused = 1;
				}
			}
			else if (trace_next_trigger == 1)
			{
				if ((cpu65.sphi | cpu65.s) == orig_sp )
				{
					trace_next_trigger = 0;
					paused = 1;
				}
