...
static void _umon_write_size_panic ( comms_details_type *cd )
{
	DEBUGPRINT("UARTMON: warning: too long message (%d/%d), cannot fit into the output buffer!" NL, cd->umon_write_pos, UMON_WRITE_BUFFER_SIZE);
}
...
int write_to_socket(comms_details_type *cd)
{
  ...
  if (ret != XS_SOCKET_ERROR || (ret == XS_SOCKET_ERROR && !xemusock_should_repeat_from_error(xerr)))
    DEBUG("UARTMON: write(" PRINTF_SOCK ",buffer+%d,%d)=%d (%s)" NL,
      (Sint64)cd->sock_client, cd->umon_write_pos, cd->umon_write_size,
